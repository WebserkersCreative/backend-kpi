...
// ========================= CORS =========================
const FRONTEND_URL = "https://webserkerscreative.github.io/Indikator_KPI/";
app.use(
  cors({
    origin: FRONTEND_URL,
...
app.post("/api/register", async (req, res) => {
  try {
    ...
    if (!email || !password || !name) {
      return res
        .status(400)
        .json({ result: "error", message: "Email, password, dan nama wajib diisi!" });
    }

    const response = await axios.post(GOOGLE_SCRIPT_URL, { action: "register", email, password, name });
...
app.post("/api/kpi-batch", async (req, res) => {
  try {
    ...
    if (!Array.isArray(indikator_list) || indikator_list.length === 0) {
      return res.status(400).json({ result: "error", message: "Indikator KPI tidak valid." });
    }

    const indikatorResponse = await axios.post(GOOGLE_SCRIPT_URL, { action: "getIndikatorData" });
    if (indikatorResponse.data.result !== "success") {
      return res.status(500).json({ result: "error", message: "Gagal validasi indikator (master data)." });
    }
    ...
    for (const item of indikator_list) {
      ...
      if (!master) {
        return res.status(400).json({ result: "error", message: `Indikator "${item.indikator_kpi}" tidak ditemukan.` });
      }
    ...
    res.status(500).json({ result: "error", message: "Gagal mengirim KPI." });
  }
});
...
app.get("/api/indikator-data", async (req, res) => {
  try {
    const response = await axios.post(GOOGLE_SCRIPT_URL, { action: "getIndikatorData" });
    res.json(response.data);
  } catch (error) {
    console.error("❌ Error indikator:", error.message);
    res.status(500).json({ result: "error", message: "Gagal mengambil indikator!" });
  }
});
...
app.get("/api/kpi-my", async (req, res) => {
  try {
    ...
    if (!email) return res.status(400).json({ result: "error", message: "Email wajib dikirim" });

    const response = await axios.post(GOOGLE_SCRIPT_URL, { action: "getKpiByUser", email });
    res.json(response.data);
  } catch (error) {
    console.error("❌ Error get KPI user:", error.response?.data || error.message);
    res.status(500).json({ result: "error", message: "Gagal mengambil KPI user" });
  }
});
...
app.post("/api/kpi-update", upload.single("buktiFile"), async (req, res) => {
  try {
    ...
    if (!kpiKey || !email) return res.status(400).json({ result: "error", message: "ID KPI dan email wajib dikirim!" });
    ...
    res.status(500).json({ result: "error", message: "Gagal update KPI" });
  }
});
...
app.post("/api/team-kpi", async (req, res) => {
  try {
    ...
    if (!nama || typeof nama !== "string")
      return res.status(400).json({ result: "error", message: "Nama viewer wajib dikirim" });

    nama = nama.trim();
    const response = await axios.post(GOOGLE_SCRIPT_URL, { action: "getTeamKPI", nama }, { timeout: 15000 });

    if (!response.data || response.data.result !== "success") {
      return res.status(400).json(response.data || { result: "error", message: "Gagal mengambil KPI team" });
    }

    res.json(response.data);
  } catch (error) {
    console.error("❌ Error get team KPI:", error.response?.data || error.message);
    res.status(500).json({ result: "error", message: "Server gagal memproses KPI team" });
  }
});
